 ...
 * 
 * gcc $(pkg-config --cflags libadwaita-1) -o emptywindow emptywindow.c  $(pkg-config --libs libadwaita-1)
 * 
 * Beispiel:
 * adw_application_window_set_content = Adwaita-Funktion;
 * GTK_WIDGET() Gtk-Macro, konvertiert einen spezifischen Widget-Typ in den allgemeinen GtkWidget-Typ;
 * AdwToolbarView = Adwaita-Widget
 *
 * 
 * Version 0.4   2025-11-28
 * 
 * Please note:
...
static void on_activate (AdwApplication *app, gpointer)
{
    ...
    gtk_window_set_title (GTK_WINDOW(win), "emptywindow");        // der WM-Title
    ...
    /* ----- Fenstertitel mit Adwaita .heading Class ------ */
    GtkWidget * title_label = gtk_label_new ("My First Window");  // Label für Fenstertitel
    ...
    gtk_window_set_default_size (GTK_WINDOW(win), 640, 400);      // Fenstergröße

    /* ----- Schaltfläche erzeugen ----- */
    GtkWidget *first_btn = gtk_button_new_with_label("Press me");

    /* ----- Schaltfläche links in die Headerbar setzen ----- */
    adw_header_bar_pack_start(header, first_btn);

    /* ----- Schaltfläche in Theme-Akzent-Farbe ----- */
    gtk_widget_add_css_class (first_btn, "suggested-action");

    /* ----- Headerbar der Toolbarview hinzufügen ----- */
    adw_toolbar_view_add_top_bar (toolbar_view, GTK_WIDGET (header));

    /* ----- Schalflächen verbinden ----- */
    ...
    // hier kann noch mehr hinzugefügt werden...
...
int main (int argc, char **argv)
{
    ...
    g_autoptr (AdwApplication) app = adw_application_new ("free.toq.window", G_APPLICATION_DEFAULT_FLAGS);
